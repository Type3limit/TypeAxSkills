...
description: |
  ...
  模糊归因、破折号过度使用、三段式法则、AI 词汇、否定式排比、过多连接词、中文定语堆叠（翻译腔/名词前长修饰语）。适用于说明文、百科、营销文。
allowed-tools:
  - Read
  - Write
  - Edit
...
### 中文专属 · 定语堆叠症（左分支堆叠 / 翻译腔）

**问题：** 这是中文 AI 写作最显眼的毛病之一，尤其在小说里。AI（多在英文语料上训练）习惯**左分支**，把一长串修饰语全堆在名词左边：“那几天没洗的油头”“这标签页拉满、还烫手的电脑”“他看着那扇锈迹斑斑、散发着恶臭的铁门”。好的中文是**右分支、动词驱动、名词先行**：先把东西摆出来，再用独立短句或谓语去描述它的状态。单纯“少用形容词”治不了：AI 会把形容词换成名词性修饰语，照堆不误。要改的不是删形容词，是**句法重组**。

**三条操作规则：**

1. **名词先行，描写后置。** 名词前的修饰语别超过六七个字。要描述状态，先点出名词，再另起短句或谓语补上。
2. **管住“的”。** 一个分句里别出现两个以上的“的”；尤其严禁“长修饰语＋的＋名词”这种结构。
3. **用动词，别用静态属性。** 把状态结合进人物的动作或感官，让它“发生”出来，而不是挂在名词前面。

**改写前（左分支堆叠）：**
> 他按下了那个因为挂机太久而发烫的手机屏幕。她坐进那张死硬冰凉的真皮沙发，望着窗外那片被夕阳染红、缓缓飘动的云。

**改写后（名词先行 / 动词驱动）：**
> 他按亮手机。挂机太久，屏幕有点烫手。她坐进真皮沙发，又硬又凉，硌得慌。窗外的云被夕阳染红了，飘得很慢。

**核心逻辑差异：**
- AI 的逻辑：物 ＝ {属性一、属性二、属性三} ＋ 名词。
- 要的逻辑：物先在场，人去看它、碰它，于是属性一冒出来，属性二被感到。
...
- ✓ **使用了"此外""然而"等连接词？** 考虑删除
- ✓ **三段式列举？** 改为两项或四项
- ✓ **名词前修饰语超过六七个字？** 拆成名词后的独立短句或动作（名词先行、右分支）
- ✓ **一个分句里两个以上“的”？** 拆句，把长定语改成谓语或动词
